...
function loadWebviewFiles(root) {
  let main = fs.readFileSync(path.join(root, 'board', 'LayerStack.html'), { encoding: 'utf8' })
...
function activate(context) {
  ...
  function pushCurrentLine() {
    ...
    if (typeof text === 'string' && currentPanel) {
      ...
      if(!filename.endsWith('.ls.json')){
        let match = /['"][^'"]+\.ls\.json['"]/.exec(text)
        if(match){
          ...
          if (path.isAbsolute(matchname)) {
            ...
            if (/loadfromlib/.exec(text)) {
              filename=path.join(dir,'../gdsql',matchname);
          ...
          vscode.window.showErrorMessage('No valid filename as .ls.json');
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('AttachmentTree.editCurrentLineAsLayerStack', () => {
      if (currentPanel) {
        showPanel()
      } else {
        createNewPanel()
...
function getWebviewContent(cdnpath) {
  ...
  Array.from(vscode.workspace.getConfiguration('AttachmentTree')['customized']).forEach((operate)=>{
    if (operate.type === 'dynamiclist') {
      ret=ret.replaceAll("'blockly_dynamic_list_inject_token'",operate.dynamiclist)
    }
  })
  return ret
}